...
from strategies.signal_engine import generate_composite_signal, generate_elite_signal
...
def scan_for_signals(data_client, risk_manager: RiskManager, 
    ...
    for symbol in config.WATCHLIST:
        try:
            ...
            # Generate ELITE signal (ARIMA + GARCH + MC + Bayes + ML + Physics + Sentiment)
            signal_data = generate_elite_signal(df, config, symbol, signal_history)
            ...
            # Entry price and stops — use fluid dynamics support/resistance if available
            entry     = signal_data['close']
            atr_val   = signal_data['atr']
            direction = signal_data['direction']
            support    = signal_data.get('support', None) or None
            resistance = signal_data.get('resistance', None) or None

            # Calculate stops using physics-derived key levels
            ...
            # Size the position — apply GARCH volatility multiplier
            ...
            # Apply GARCH vol regime multiplier to shares
