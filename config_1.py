...
class SignalConfig:
    ...
    factor_weights: Dict[str, float] = field(default_factory=lambda: {
        # --- Momentum cluster (40%) ---
        "roc_5d":               0.08,   # 5-day rate of change
        "roc_10d":              0.07,   # 10-day rate of change
        "macd_histogram":       0.08,   # MACD(12,26,9) histogram slope
        "adx_trend_strength":   0.06,   # ADX > 20 = trending
        "ema_crossover":        0.06,   # 8/21 EMA cross recency & magnitude
        "price_vs_vwap":        0.05,   # Daily close vs 5d rolling VWAP

        # --- Mean-reversion cluster (20%) ---
        "rsi_swing":            0.07,   # RSI(5) oversold bounce / overbought fade
        "bollinger_reversion":  0.07,   # % distance from Bollinger(20,2) bands
        "stochastic_cross":     0.06,   # Stoch %K/%D cross in extreme zones

        # --- Volume / Microstructure (15%) ---
        "volume_surge":         0.06,   # Volume vs 20d avg (breakout confirm)
        "obv_divergence":       0.05,   # OBV trend vs price trend divergence
        "vwap_reclaim":         0.04,   # Intraday VWAP reclaim on daily close

        # --- Macro / Regime (15%) ---
        "fred_regime_tilt":     0.06,   # FRED composite macro score
        "vix_regime":           0.05,   # VIX level & direction
    ...
    # Composite scoring
    min_composite_score: float = 0.55   # Minimum to generate a BUY signal
    min_factors_positive: int = 8       # At least 8 of 16 factors must be positive
