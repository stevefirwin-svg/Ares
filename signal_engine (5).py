...
# =============================================================================
# ULTIMATE SIGNAL v4.0 — ALL SYSTEMS COMBINED
# =============================================================================

def generate_ultimate_signal(df, config, symbol="", signal_history=None):
    """
    The most comprehensive signal available combining:
    - 17-point technical/quant/physics/psychology/sentiment system
    - Sector-aware strategy weights
    - Renaissance models (HMM, Kalman, Fourier, Entropy, IC)
    - Volume Profile & POC (institutional levels)
    - Smart Money / Options Flow proxy
    - LSTM-inspired Pattern Recognition
    - Market Breadth indicators
    - Adaptive conviction-based sizing
    """
    if signal_history is None:
        signal_history = []

    # Layer 1: Renaissance signal (our best existing system)
    base = generate_renaissance_signal(df, config, symbol, signal_history)

    # Layer 2: Advanced signals
    advanced = {"composite_signal": 0.5, "smart_money_bullish": False,
                "pattern_direction": "NEUTRAL", "breadth_regime": "NEUTRAL",
                "poc_price": 0, "value_area_high": 0, "value_area_low": 0}
    try:
        from models.advanced_signals import run_advanced_signals
        advanced = run_advanced_signals(df, symbol, config)
    except Exception as e:
        logger.debug(f"Advanced signals unavailable: {e}")

    # Layer 3: Finnhub real sentiment
    finnhub_result = {"signal": 0.5, "category": "NEUTRAL",
                      "key_signals": {}}
    try:
        from sentiment.finnhub_sentiment import get_finnhub_analyzer
        fh = get_finnhub_analyzer(config)
        if fh:
            current_price = float(df['close'].iloc[-1])
            finnhub_result = fh.get_full_sentiment(symbol, current_price)
    except Exception as e:
        logger.debug(f"Finnhub unavailable: {e}")

    # Blend signals — Finnhub adds real-world fundamental layer
    base_score     = base.get("signal_score", 0.5)
    advanced_score = advanced.get("composite_signal", 0.5)
    finnhub_score  = finnhub_result.get("signal", 0.5)

    # If Finnhub is configured, include it. Otherwise use 50/50 base/advanced
    finnhub_key = getattr(config, 'FINNHUB_API_KEY', '')
    if finnhub_key and finnhub_key != "YOUR_FINNHUB_API_KEY_HERE":
        blended = (base_score     * 0.50 +
                   advanced_score * 0.30 +
                   finnhub_score  * 0.20)
    else:
        blended = base_score * 0.60 + advanced_score * 0.40

    blended = max(0.0, min(1.0, blended))

    # Smart money confirmation bonus
    if advanced.get("smart_money_bullish") and blended > 0.55:
        blended = min(1.0, blended + 0.05)

    # Finnhub insider buying = very strong signal
    insider = finnhub_result.get("insider_activity", {})
    if isinstance(insider, dict):
        if insider.get("exec_buying"):
            blended = min(1.0, blended + 0.08)  # CEO buying = big boost
        elif insider.get("activity") == "CLUSTER_BUYING":
            blended = min(1.0, blended + 0.06)

    # Finnhub analyst strong sell = override warning
    analyst = finnhub_result.get("analyst_signals", {})
    if isinstance(analyst, dict):
        if analyst.get("recommendation") == "STRONG_BUY" and blended > 0.55:
            blended = min(1.0, blended + 0.04)
        elif analyst.get("recommendation") == "SELL":
            blended = max(0.0, blended - 0.05)

    # Pattern alignment bonus
    if advanced.get("pattern_direction") == "UP" and blended > 0.55:
        blended = min(1.0, blended + 0.03)

    # Breadth penalty
    if advanced.get("breadth_regime") == "STRONG_BEAR":
        blended *= 0.75

    # Recalculate score
    score_17 = blended * 17
    if score_17 >= 15:   grade = "ELITE"
    elif score_17 >= 12: grade = "STRONG"
    elif score_17 >= 9:  grade = "GOOD"
    elif score_17 >= 6:  grade = "MARGINAL"
    else:                grade = "NO TRADE"

    win_prob = 0.45 + (blended * 0.32)
    ...
    base.update({
        ...
        "poc_price":           advanced.get("poc_price", 0),
        "value_area_high":     advanced.get("value_area_high", 0),
        "value_area_low":      advanced.get("value_area_low", 0),
        "smart_money_bullish": advanced.get("smart_money_bullish", False),
        "pattern_direction":   advanced.get("pattern_direction", "NEUTRAL"),
        "breadth_regime":      advanced.get("breadth_regime", "NEUTRAL"),
        "news_sentiment":      finnhub_result.get("category", "NEUTRAL"),
        "insider_activity":    finnhub_result.get("key_signals", {}).get("insider", "NONE"),
        "analyst_rec":         finnhub_result.get("key_signals", {}).get("analyst", "NEUTRAL"),
        "upside_to_target":    finnhub_result.get("key_signals", {}).get("upside_to_target", "N/A"),
    })
