...
from strategies.signal_engine import (generate_composite_signal,
    generate_elite_signal, generate_sector_aware_signal,
    generate_renaissance_signal, generate_ultimate_signal)
...
from models.advanced_signals import AdaptiveStopLoss
...
def process_symbol(symbol: str, data_client, risk_manager: RiskManager,
    ...
    try:
        ...
        signal_data = generate_ultimate_signal(df, config, symbol, signal_history)
        ...
        # Calculate stops using adaptive system
        garch_r    = signal_data.get('quant', {}).get('garch', {}).get('regime', 'NORMAL')
        adaptive   = AdaptiveStopLoss(config)
        hist_vol   = df['hist_vol'].iloc[-1] if 'hist_vol' in df.columns else 20.0
        vol_regime = "LOW" if hist_vol < 15 else "NORMAL" if hist_vol < 25 \
                     else "ELEVATED" if hist_vol < 35 else "HIGH"
        adaptive_stops = adaptive.calculate_adaptive_stops(
            entry_price=entry,
            direction=direction,
            atr_value=atr_val,
            conviction_score=signal_data.get('signal_score', 0.5),
            volatility_regime=vol_regime,
        )
        stop   = adaptive_stops['initial_stop']
        target = adaptive_stops['final_target']

        # Also use volume profile support/resistance if available
        poc    = signal_data.get('poc_price', 0)
        vah    = signal_data.get('value_area_high', 0)
        val    = signal_data.get('value_area_low', 0)
        if val > 0 and direction == 'long' and val < entry:
            stop = max(stop, val - atr_val * 0.2)
        if vah > 0 and direction == 'long' and vah > entry:
            target = min(target, vah + atr_val * 0.2)
        ...
        if approved:
            ...
            hmm       = signal_data.get('hmm_regime', '')
            breadth   = signal_data.get('breadth_regime', '')
            pattern   = signal_data.get('pattern_direction', '')
            sm_bull   = signal_data.get('smart_money_bullish', False)
            news_sent = signal_data.get('news_sentiment', 'NEUTRAL')
            insider   = signal_data.get('insider_activity', 'NONE')
            analyst   = signal_data.get('analyst_rec', 'NEUTRAL')
            upside    = signal_data.get('upside_to_target', 'N/A')
            ...
            logger.info(f"   HMM: {hmm} | Breadth: {breadth} | "
                       f"Pattern: {pattern} | SmartMoney: {'YES' if sm_bull else 'NO'}")
            logger.info(f"   News: {news_sent} | Insider: {insider} | "
                       f"Analyst: {analyst} | Upside: {upside}")
            return trade_signal
        else:
